...
if (!clerk.isSignedIn) {
    ...
    if (document.getElementById("protected-content").hidden===false){
        document.getElementById("default-content").hidden = false;
        document.getElementById("protected-content").hidden = true;
    }

    sessionStorage.removeItem('access_token');
    await clerk.redirectToSignIn({ redirectUrl: `${window.location.origin}/index.html` });

} else {
    await syncAccessToken();

    if (document.getElementById("protected-content").hidden){
